...
transactionService = {
    getTransactionsPublic: async (sellerId, buyerId) => {
        ...
        let transactions = [];
        if ( buyerId != null){
            let buyerTransactions = await transactionRepository.getTransactionPublicByBuyer(buyerId);
            transactions = transactions.concat(buyerTransactions)
        }
        if ( sellerId != null){
            let sellerTransactions = await transactionRepository.getTransactionPublicBySeller(sellerId);
            transactions = transactions.concat(sellerTransactions)
        }
    ...
    postTransaction: async (buyerId, productId, buyerPaymentId, startDate, endDate) => {
        // console.log("BUYER PAYMENT ID", buyerPaymentId)
        ...
        let transactionInfo = await transactionRepository.addTransaction(
            ...
            productId, product.price, buyerPaymentId, startDate, endDate
        );
