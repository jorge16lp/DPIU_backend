...
productsService = {
    ...
    putProduct: async (userId, productId, title, description, price, date, category) => {
        let errors = []
        ...
        console.log(date)
        if ( date == undefined )
            errors.push(new InputError("date",'date is undefined'));
        // Errors in client INPUTS
        if (errors.length > 0)
            throw errors
        ...
        let modified = await productRepository.editProduct(productId, title, description, price, category, date);
