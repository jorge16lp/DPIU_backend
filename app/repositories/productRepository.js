...
productsRepository = {
    ...
    editProduct: async (productId, title, description, price, category, date) => {
        let modifiedInfo = null;
        try{
            await database.connect();
            modifiedInfo = await database.query("UPDATE products SET title=?, date=?, description=?, price=?, category=? WHERE id=?",
                [title, date, description, price, category, productId])
            await database.disconnect();
        } catch (e){
            await database.disconnect();
            console.log(e) // ERROR IN DATABASE OR SQL
    ...
    getProductsSummary: async () => {
        ...
        try{
            ...
            products = await database.query("SELECT products.id, products.title, products.description, products.price, products.category," +
                "usersB.email as buyerEmail, usersB.name as buyerName, usersS.email as sellerEmail " +
                "FROM products LEFT JOIN users as usersB ON products.buyerId = usersB.id LEFT JOIN users as usersS ON products.sellerId = usersS.id")
            await database.disconnect();
        } catch (e){
            await database.disconnect();
            console.log(e) // ERROR IN DATABASE OR SQL
    ...
    getProduct: async (id) => {
        ...
        try{
            await database.connect();
            let products = await database.query("SELECT products.*, users.email as sellerEmail, users.name as sellerName " +
                "FROM products LEFT JOIN users ON products.sellerId = users.id WHERE products.id = ?",[id])
