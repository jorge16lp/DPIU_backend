...
transactionRepository = {
    getTransactionPublicBySeller: async (sellerId) => {
        ...
        try{
            ...
            transactions = await database.query("SELECT transactions.*, usersBuyer.email as buyerEmail, usersSeller.email as sellerEmail, products.*, transactions.id as tid " +
                "FROM transactions LEFT JOIN products ON products.id = transactions.productId LEFT JOIN users as usersBuyer ON transactions.buyerId = usersBuyer.id LEFT JOIN users as usersSeller ON products.sellerId = usersSeller.id " +
    ...
    getTransactions: async (userId) => {
        ...
        try{
            ...
            transactions = await database.query("SELECT transactions.*, usersBuyer.email as buyerEmail, usersSeller.email as sellerEmail, products.*, transactions.id as tid " +
                "FROM transactions LEFT JOIN products ON products.id = transactions.productId LEFT JOIN users as usersBuyer ON transactions.buyerId = usersBuyer.id LEFT JOIN users as usersSeller ON products.sellerId = usersSeller.id " +
                "WHERE transactions.buyerId = ? OR transactions.sellerId = ?",[userId,userId])
            await database.disconnect();
        } catch (e){
            await database.disconnect();
            console.log(e) // ERROR IN DATABASE OR SQL
    ...
    addTransaction: async (buyerId, sellerId, buyerCountry, buyerAddress, buyerPostCode, productId, productPrice, buyerPaymentId, startDate, endDate) => {
        let insertInfo = null;
        try{
            await database.connect();
            insertInfo = await database.query("INSERT INTO transactions (buyerId, sellerId, buyerCountry, buyerAddress, buyerPostCode, productId, productPrice, buyerPaymentId, startDate, endDate) VALUES (?,?,?,?,?,?,?,?,?,?)",
                [buyerId, sellerId, buyerCountry, buyerAddress, buyerPostCode, productId, productPrice, buyerPaymentId, startDate, endDate])
